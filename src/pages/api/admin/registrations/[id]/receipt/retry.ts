...
export function createReceiptRetryRoute(options: Options = {}) {
  const database = options.database;
  return createAuthenticatedReceiptRecoveryRoute({
    action: 'retry', sessionSecretB64: options.sessionSecretB64,
...
export const POST = async (context: any) => {
  ...
  try {
    ...
    return createReceiptRetryRoute()(context);
  }
  return createReceiptRetryRoute({ database, sessionSecretB64: import.meta.env.ADMIN_SESSION_SECRET_B64 })(context);
};
